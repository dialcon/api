...
async.auto({
      ...
      if (res.statusCode === 200) {
        ...
        request.get({
          ...
          if (res.statusCode === 200 && body.length) {
            ...
            return cb('Imposible encontrar espacio test'.red);
          }
        });
      }
    });
  ...
  modules: ['test', (results, cb) => {
    let modules = [];
    modules.push(require('./models/clientes.json'));
    modules.push(require('./models/type_poliza.json'));
    ...
    async.eachSeries(modules, (item, cb) => {
      async.auto({
        table: (cb) => {
          ...
          request.post({
            url: `${url_core}/spaces/${results.test}/modules`,
            headers: {
              'Authorization': `Bearer ${results.token}`
            },
            json: true,
            ...
            if (res.statusCode === 200) {
              ...
              console.log(body);
              request.get({
                url: `${url_core}/spaces/${results.test}/modules?name=${item.name}`,
                json: true,
                headers: {
                  'Authorization': `Bearer ${results.token}`
                }
                ...
                if (res.statusCode === 200 && body.length) {
                  ...
                  return cb(`Imposible encontrar modelo ${item.name} en test`.red);
                }
              });
            }
          });
  ...
  apikey: ['test', (results, cb) => {
    ...
    request.post({
      ...
      if (err) {
        ...
        request.get({
          url: `${url_core}/apikey/${results.test}?name=homedesk`,
          headers: {
            'Authorization': `Bearer ${results.token}`
          }
        }, (err, res, body) => {
          ...
          if (res.statusCode === 200 && body.length) {
            ...
            return cb('Imposible encontrar key homedesk en test'.red);
          }
        });
      }
    });
